...
void* data_transfer(void* arg) {
    ...
    printf("Transferring %d packets from %d to %d...\n", packets, from, to);
    ...
    FILE* f1 = fopen(from_file, "a");
    fprintf(f1, "Sent %d packets to %d\n", packets, to);
    fclose(f1);

    FILE* f2 = fopen(to_file, "a");
    fprintf(f2, "Received %d packets from %d\n", packets, from);
    fclose(f2);
...
void export_network_dot(const char* filename) {
    ...
    for (int i = 0; i < nodes; i++) {
        for (Edge* e = graph[i]; e; e = e->next) {
            if (i < e->destination) // Avoid duplicates
...
int main() {
    ...
    while (1) {
        ...
        printf("3. Add Route\n");
        printf("4. Transfer Data\n");
        printf("5. Exit\n");
        printf("6. Export Network Visualization\n");
        printf("Choice: ");
        int choice;
        scanf("%d", &choice);
